#!/usr/bin/env python3
"""Build a diagnostic-only IMG_2628 placement-action YOLO dataset.

The earlier fixed-region event dataset learned the static output area too
readily. This builder labels the visual placement action region instead: the
worker/panel area during provisional completed-placement windows, with rejected
press-handling windows and post-completion frames kept as hard negatives.

This artifact is diagnostic-only. It is not human-reviewed truth and is not
eligible for promotion/training use without separate review.
"""
...
SCHEMA_VERSION = "img2628-diagnostic-placement-action-yolo-dataset-v1"
...
DEFAULT_OUT_DIR = Path("data/labels/img2628_placement_action_diagnostic_v1")
IMAGE_WIDTH = 1920
IMAGE_HEIGHT = 1080

# Covers the operator's lower-body/panel-placement area while avoiding most of
# the static right-side rack that caused the fixed-box detector to overcount.
...
def far_from_accepted_windows(timestamp: float, accepted_events: list[float]) -> bool:
    for event_ts in accepted_events:
        if event_ts - 14.0 <= timestamp <= event_ts + 10.0:
            return False
    return True
...
def build_dataset(*, worksheet: Path, video: Path, out_dir: Path, force: bool) -> Path:
    ...
    try:
        ...
        for row in accepted:
            ...
            # Label the action lead-in and completion posture; post-completion
            # stillness is negative so the model does not learn the static pile.
            for offset in (-4.0, -2.0, 0.0):
                ...
                add_item(
                    ...
                    reason="diagnostic_positive_placement_action_window",
            ...
            for offset in (4.0, 8.0, 12.0):
                ...
                add_item(
                    ...
                    reason="post_completion_static_or_walkaway_negative",
                    box=None,
                )
                item_index += 1
        ...
        for row in rejected:
            ...
            for offset in (-4.0, -2.0, 0.0, 2.0, 4.0):
                ...
                add_item(
                    ...
                    reason="rejected_press_or_continuation_motion_negative",
                    box=None,
                )
                item_index += 1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build diagnostic-only IMG_2628 placement-action YOLO dataset")
