...
export function jobPaceSentence(job: JobSeed, snapshot: PaceSnapshot, now: Date) {
  ...
  const expected = Math.round(snapshot.expected_units_by_now);
  ...
  if (now > new Date(job.deadline)) {
    return `${done} of ${job.units_required} done - overdue. At this pace labor eats the margin.`;
  }
  if (snapshot.pace_delta < -0.05 * Math.max(snapshot.expected_units_by_now, 1)) {
    return `${done} of ${job.units_required} done - you need ${expected} by now. Needs ${neededPerDay}/day, doing ${actualPerDay}.`;
  }
  return `${done} of ${job.units_required} done - you need ${expected} by now. Finishes Monday morning.`;
}
