...
def build_total_payload(
    *,
    ...
    return {
        ...
        "verification_status": "provisional_total_only",
        "timestamp_truth_status": "not_reviewed_yet",
        "validation_note": (
    ...
    *,
    ...
    return {
        ...
        "truth": {
            ...
            "notes": [
                "Bootstrap candidate manifest only.",
...
def build_next_steps(*, case_id: str, video_path: Path, expected_total: int, baseline_case_id: str | None) -> list[str]:
    ...
    return [
        ...
        f"3. Use the human total {expected_total} as the target, not as proof.",
...
def bootstrap(args: argparse.Namespace) -> dict[str, Any]:
    ...
    truth_csv_path = reports_dir / f"{stem}_human_truth_event_times.template.csv"
    ...
    total_payload = build_total_payload(
        ...
        human_counter=args.human_counter,
        truth_rule_id=args.truth_rule_id,
    ...
    if not args.dry_run:
        ...
        write_truth_template(truth_csv_path, case_id=args.case_id, expected_total=args.expected_total)
        if args.preview:
            subprocess.run(
                [
                    ".venv/bin/python",
