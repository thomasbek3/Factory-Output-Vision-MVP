-- Storage RLS cannot inspect server-only lineage tables as the reviewer role.
-- Keep that lineage private and expose only this assignment-bound yes/no check.
...
drop policy if exists reviewer_read_active_assignment_media on storage.objects;
