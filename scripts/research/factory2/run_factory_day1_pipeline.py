...
def main(argv: list[str] | None = None) -> int:
    ...
    stages = [
        ...
        ("review", [PYTHON, "scripts/research/factory2/review_labels_ai.py", str(auto_boxes), "--output", str(reviewed)]),
        ("negatives", [PYTHON, "scripts/research/factory2/export_onboarding_stable_negatives.py", "--event-proposals", str(proposals), "--work-dir", str(work / "negatives"), "--out-dir", str(hard_negatives_dir), "--force"]),
        ("dataset", [PYTHON, "scripts/research/factory2/assemble_active_panel_dataset.py", "--reviewed-label-manifest", str(reviewed), "--hard-negative-export", str(hard_negatives_dir / "hard_negative_export.json"), "--out-dir", str(dataset_dir), "--force"]),
        ("train", [PYTHON, "scripts/research/factory2/run_yolo26_training_eval.py", "--data-yaml", str(dataset_dir / "data.yaml"), "--dataset-manifest", str(dataset_dir / "dataset_manifest.json"), "--base-model", str(args.base_model), "--train-project", str(work / "train"), "--train-name", "model", "--epochs", str(args.epochs), "--device", args.device, "--execute-training", "--output", str(training_report), "--force"]),
    ...
    print(json.dumps({
        ...
        "next": "scrub exam_clip, collect placement times, then scripts/research/factory2/build_day1_exam_gate.py",
