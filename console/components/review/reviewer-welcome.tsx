...
export function ReviewerWelcome() {
  ...
  useEffect(() => {
    ...
    void acceptReviewerInviteSession(accessToken, refreshToken)
      .then(() => {
      ...
      <div className="w-full max-w-md border border-[var(--border)] bg-[var(--panel)] p-7 text-center">
        {state === "loading" ? (
          <>
            ...
            <h1 className="mt-5 text-[22px] font-semibold">Securing your invitation</h1>
            <p className="mt-2 text-[13px] leading-5 text-[var(--text-mut)]">This takes only a moment.</p>
          </>
        ) : state === "ready" ? (
          <>
            <CheckCircle2 className="mx-auto h-8 w-8 text-[var(--good)]" />
            <h1 className="mt-5 text-[22px] font-semibold">Invitation accepted</h1>
            <p className="mt-2 text-[13px] leading-5 text-[var(--text-mut)]">Opening your secure setup now.</p>
          </>
        ) : (
          <>
            <ShieldAlert className="mx-auto h-8 w-8 text-[var(--bad)]" />
            <h1 className="mt-5 text-[22px] font-semibold">This invitation is unavailable</h1>
            <p className="mt-2 text-[13px] leading-5 text-[var(--text-mut)]">
              It may have expired or already been used. Ask FactoryVision support for a new invitation.
            </p>
          </>
        )}
      </div>
