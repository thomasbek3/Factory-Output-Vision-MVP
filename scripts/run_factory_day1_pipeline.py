...
def cap_event_proposals(proposals_path: Path, *, max_events: int) -> dict[str, int]:
    ...
    if len(events) <= max_events:
        return {"events_total": len(events), "events_kept": len(events)}
    stride_indices = sorted({round(i * (len(events) - 1) / (max_events - 1)) for i in range(max_events)})
    kept = [events[i] for i in stride_indices]
    payload["proposals"] = kept + negatives
    ...
    return {"events_total": len(events), "events_kept": len(kept)}
...
def main(argv: list[str] | None = None) -> int:
    ...
    stages = [
        ("proposals", [PYTHON, "scripts/propose_onboarding_events.py", "--segment-manifest", str(train_manifest), "--output", str(proposals), "--sample-fps", "2", "--motion-threshold", "0.01", "--min-cluster-gap-sec", "1.5", "--window-before-sec", "6", "--window-after-sec", "6", "--stable-negative-count", "3", "--force"]),
        ("packets", [PYTHON, "scripts/build_teacher_evidence_packets.py", "--event-proposals", str(proposals), "--output-dir", str(packets_dir), "--sequence-fps", "2", "--max-width", "960", "--force"]),
        ("teacher", teacher_cmd),
    ...
    for name, command in stages:
        ...
        if name == "proposals":
            cap = cap_event_proposals(proposals, max_events=args.max_teacher_events)
