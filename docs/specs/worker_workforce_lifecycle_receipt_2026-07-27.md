...
- Supabase Auth now uses the Vercel portal as its Site URL and includes the
  portal wildcard in its redirect allowlist.
- Thomas's permanent ops account has a configured password and completed a
  live authenticated `/ops` login. The credential is not stored in the
  repository or this receipt.
...
2. Configure production email delivery, send a controlled worker invitation,
   and prove its activate-account link.
3. Decide whether ops must enforce MFA, then run one full first-worker rehearsal
   through qualification and three-person production resolution.
...
- The permanent ops account and password login are proven, but ops MFA is not
  currently enforced or enrolled.
