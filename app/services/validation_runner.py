"""Manifest-backed real-app validation workflow (moved from scripts/validate_video.py).

CP6 inverts the app->scripts dependency: app/services/blind_replay_gate.py
used to import `scripts.validate_video.run_validation`, putting an app-layer
module beneath a CLI. The orchestration now lives here; scripts/validate_video
remains as a thin argument-parsing wrapper over run_validation.

Truth contract (unchanged): validation_truth_guard.validate_truth_file still
gates every execute; teacher labels are never used as runtime truth.
"""

from __future__ import annotations

import json
import subprocess  # noqa: S404 - commands are built from repo-owned manifests
import sys
import time
